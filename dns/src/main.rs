    ...
    pub fn write(&self, buffer: &mut BytePacketBuffer) -> Result<usize, Box<dyn Error>> {
        ...
        match *self {
            ...
            DnsRecord::AAAA {
                ...
                for octect in &addr.segments() {
                    buffer.write_u16(*octect)?;
                }
            }
...
fn recursive_lookup(qname: &str, qtype: QueryType) -> Result<DnsPacket, Box<dyn Error>> {
    let mut ns = "192.41.0.4".parse::<Ipv4Addr>().unwrap();
...
fn handle_query(socket: &UdpSocket) -> Result<(), Box<dyn Error>> {
    ...
    let data = res_buffer.get_buf_range(0, len)?;
